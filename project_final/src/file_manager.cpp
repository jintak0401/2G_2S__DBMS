...
int fd[11] = {-3, -3, -3, -3, -3, -3, -3, -3, -3, -3, -3};
char fd_name[10][100];
...
int file_read_page(int table_id, pagenum_t page_num, page_t dest){
  ...
  if (table_id > fd_size || table_id < 0 || fd[table_id] == -3){
    perror("calling not exist table_id");
    exit(EXIT_FAILURE);
  }
...
int file_write_for_join(char* pathname, pagenum_t page_num, char src[PAGE_SIZE], int phase){

  int table_id = 0, ret_val = 0;

  if ((phase == 2 && fd[table_id] == -3) || phase == 0){
    fd[table_id] = open(pathname, O_WRONLY | O_TRUNC | O_CREAT, 0644);
  }

  if (fd[table_id] <= 0){
    perror("cannot join at [ file_manager.c ], in < file_write_for_join >");
    exit(EXIT_FAILURE);
  }
  
  ret_val = pwrite (fd[table_id], src, PAGE_SIZE, PAGE_SIZE * page_num);
...
void file_read_for_join(char* pathname){

  int table_id = 0;
  char str[PAGE_SIZE];
  pagenum_t page_num = 0;

  int check = 0;

  fd[table_id] = open(pathname, O_RDONLY | O_CREAT, 0644);

  if (fd[table_id] == -3){
    perror("[file_manager.c], < file_read_page > ");
    exit(EXIT_FAILURE);
  }

  while (pread(fd[table_id], str, PAGE_SIZE, PAGE_SIZE * page_num) > 0){
    printf("%s", str);
    str[0] = '\0';
    page_num++;
  }
 ...
 */
  ...
  if (strcmp(pathname, (char*)"DATA1") == 0){
    table_id = 1;
  }
  else if (strcmp(pathname, (char*)"DATA2") == 0){
    table_id = 2;
  }
  else if (strcmp(pathname, (char*)"DATA3") == 0){
    table_id = 3;
  }
  else if (strcmp(pathname, (char*)"DATA4") == 0){
    table_id = 4;
  }
  else if (strcmp(pathname, (char*)"DATA5") == 0){
    table_id = 5;
  }
  else if (strcmp(pathname, (char*)"DATA6") == 0){
    table_id = 6;
  }
  else if (strcmp(pathname, (char*)"DATA7") == 0){
    table_id = 7;
  }
  else if (strcmp(pathname, (char*)"DATA8") == 0){
    table_id = 8;
  }
  else if (strcmp(pathname, (char*)"DATA9") == 0){
    table_id = 9;
  }
  else {
    table_id = 10;
